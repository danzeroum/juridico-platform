...
health: ## Health check de todos os serviços
	...
	@echo ""
	@echo "=== Endpoints ==="
	@curl -sf http://localhost:8000/api/v1/health > /dev/null && echo "✅ Gateway (liveness): OK" || echo "❌ Gateway (liveness): FAIL"
	@curl -sf http://localhost:8000/api/v1/ready > /dev/null && echo "✅ Gateway (readiness): OK" || echo "❌ Gateway (readiness): FAIL (dependências fora — veja curl localhost:8000/api/v1/ready)"
