...
| P0-2 | **Login não valida senha** — endpoint emitia JWT sem checar credenciais. | Segurança | ✅ **Implementado** — `auth.py` valida e-mail/senha/tenant contra `tenant.users` (hash PBKDF2-HMAC-SHA256, stdlib); migração `002` adiciona `password_hash` + semeia usuário dev (`admin@dev.com` / `dev12345`, só `dev-tenant`). `GET /api/v1/auth/me` + hidratação do `role`/`tenant` no frontend (fim do admin-para-todos). ⏳ **Falta:** gestão de usuários (criar/convidar — P1), refresh token (P1-9), verificação de assinatura RS256 no `middleware.ts` do frontend (hoje só presença). |
...
| P0-10 | **Sem pipeline de deploy** — nenhum build/push/scan de imagem; sem manifests K8s/Helm. | Infra/CI | 🟡 **Parcial** — CI faz **build da imagem do gateway** (gate real) + **scan Trivy** (vuln+segredos, report-only) e o compose roda **migrações no startup** (serviço `migrate` one-shot). ⏳ **Falta (decisão sua):** push para registry + deploy real — receita pronta em `docs/DEPLOY.md` (GHCR/Docker Hub/SSH); endurecer o `exit-code` do Trivy após triagem. |
